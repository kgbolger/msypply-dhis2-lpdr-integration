...
response_log_str = 'Monthly_SI_Transaction_DataSet_response_' + str(today)
response_log = open('C:\scripts\dhis2\ETL\logs\\'+response_log_str,'w') #open the response log
Monthly_DataSet_file = 'C:\scripts\dhis2\ETL\logs\Monthly_SI_Transaction_DataSet_Data_'+str(today)+'.json' #create a log for the JSON payload
...
# Get Last months SI totals (aggreate SUM of transaction quantity for transaction type SI)
url = "https://dhis2.co/msupply/api/analytics/events/aggregate/EgXN4cJ004K.json?stage=OiXz9SI5hGz&dimension=ou:"+storeIUDs+"&dimension=pe:LAST_MONTH&dimension=tnY9kPc9jTh&dimension=urAwnJWpKYj:eq:si&value=y5gmjQRi54V&aggregationType=SUM" #define the DHIS2 API url for the events, org units will be identified via mSupply codes
...
for node in json_data["rows"]:
	...
	value["dataElement"] = getReceivedBalUID(node[0])
...
url = "https://dhis2.co/msupply/api/26/dataValueSets?importStrategy=CREATE_AND_UPDATE&dryRun=FALSE" #define the DHIS2 API url for the events, org units will be identified via mSupply codes
headers = {'Content-type': 'application/json'} #define the payload as JSON
r = requests.post(url, data=j, headers=headers, auth=HTTPBasicAuth('kevinb', 'Ireland15?'), verify = False) #POST with basic authentication

...
response_log_str = 'Monthly_Closing_DataSet_response_' + str(today)
response_log = open('C:\scripts\dhis2\ETL\logs\\'+response_log_str,'w') #open the response log
Monthly_DataSet_file = 'C:\scripts\dhis2\ETL\logs\Monthly_Closing_DataSet_'+str(today)+'.json' #create a log for the JSON payload
f = open(Monthly_DataSet_file,'w') #open the log for writing

# Get todays stock on hand aggreated by item code for this end of previous month - closing stock balance
url = "https://dhis2.co/msupply/api/analytics/events/aggregate/YScaWpjOOnI.json?stage=VgrIh62C2gl&dimension=ou:"+ storeIUDs +"&dimension=pe:"+today+"&dimension=tnY9kPc9jTh&value=GMxEEg91qGO&aggregationType=SUM" #define the DHIS2 API url for the events, org units will be identified via mSupply codes
...
for node in json_data["rows"]:
	...
	value["dataElement"] = getClosingBalUID(node[0])
